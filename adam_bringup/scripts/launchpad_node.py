...
#Subscribers and Publishers of IMU data topic
		...
		self.frame_id = '/base_link'
        	...
        	#self.range_msg.field_of_view = 30.0 * (3.1416 / 180.0)
        	#self.range_msg.min_range = 0.02
       	 	#self.range_msg.max_range = 4.0
       	 	#self.range_msg.range = 0

        	self.range_pub = rospy.Publisher('/sonar', Range,queue_size = 10)
...
#######################################################################################################################
				...
				if(lineParts[0] == 'u'):
						...
						r.frame_id = "cliff_sensor_front_link"
						
						range_msg.header = r
					
						range_msg.radiation_type = Range.ULTRASOUND

	        				range_msg.field_of_view = 15.0 * (3.1416 / 180.0)
	        				range_msg.min_range = 0.02
	        				range_msg.max_range = 1.0

		        			if ((self._ultrasonic_value)/100.0) > 1.0:
		        					range_msg.range= 1.0
		        			else:
									range_msg.range = ((self._ultrasonic_value)/100.0)
